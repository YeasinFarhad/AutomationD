...
public class Reusable_Library_Loggers_POM {
    ...
    public static void dropDownByText(WebDriver driver, WebElement locator, String userValue, int index, ExtentTest loggers, String elementName) throws IOException, InterruptedException {
        WebDriverWait wait = new WebDriverWait(driver, 10);
    ...
    public static void comparePageTitle(WebDriver driver, String expectedTitle,  ExtentTest loggers, String pageName) throws IOException, InterruptedException {
        WebDriverWait wait = new WebDriverWait(driver, 10);

        String actualTitle = driver.getTitle();
        if(actualTitle.equals(expectedTitle)){
            System.out.println(pageName+" title matches. The title is "+expectedTitle);
            loggers.log(LogStatus.PASS,"Title Match. The title is "+expectedTitle);
        } else {
            System.out.println(pageName+" title does not match. Expected title: "+expectedTitle+" . --and actual title: "+actualTitle);
            loggers.log(LogStatus.FAIL,"Title does not match.Expected title: "+expectedTitle+" . --and actual title: "+actualTitle);
            getScreenshot(driver, loggers, pageName);
        }
